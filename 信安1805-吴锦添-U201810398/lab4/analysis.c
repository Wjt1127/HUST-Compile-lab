...
int array_access(struct ASTNode* root)//返回数组类型
{
    ...
    for(int i = this_dim - 2;i >= 0;i--)//循环dim-1次
    {
        ...
        result.offset = symbolTable.symbols[root->place].offset;
        result.kind = ID;

        opn2.kind = INT;
        opn2.offset = symbolTable.symbols[value].offset;
        opn2.const_int = symbolTable.symbols[value].info->diminfo[i];

        opn1.kind = ID;
        opn1.offset = symbolTable.symbols[Node_Array[i + 1]->place].offset;
        ...
        tmp = fill_Temp(newTemp(),LEV,INT,'T',symbolTable.symbols[symbolTable.index-1].offset+4);
        ...
        opn2.kind = ID;
        opn2.offset = symbolTable.symbols[Node_Array[i]->place].offset;
        strcpy(opn2.id,symbolTable.symbols[Node_Array[i]->place].alias);

        opn1.kind = ID;
        opn1.offset = symbolTable.symbols[root->place].offset;
    ...
    int tmp1 = fill_Temp(newTemp(), LEV, INT, 'T', symbolTable.symbols[symbolTable.index-1].offset+4);

    strcpy(result.id, symbolTable.symbols[tmp1].alias);
    result.offset = symbolTable.symbols[tmp1].offset;
    result.kind = ID;
    ...
    if(this_dim==1)
    {
        ...
        opn1.offset = symbolTable.symbols[Node_Array[this_dim - 1]->place].offset;
    }
    else
    {
        strcpy(opn1.id, symbolTable.symbols[tmp].alias);
        opn1.offset = symbolTable.symbols[tmp].offset;
    }
    //printf("!!!!%d\n",opn1.offset);
    T1->code = merge(2, T1->code, genIR(STAR, opn1, opn2, result));
...
void prnIR(struct codenode *head){
    ...
    do {
        ...
        if (h->opn1.kind==ARRAY)
             sprintf(opnstr1,"!!!%s[%s]",h->opn1.id,h->opn2.id);
        if (h->opn1.kind==ID)
...
void ext_var_list(struct ASTNode *T){  //处理变量列表
    ...
    switch (T->kind){
        ...
        case ID:
            rtn=fillSymbolTable(T->type_id,newAlias(),LEV,T->type,'Q',T->offset);  //最后一个变量名
            if (rtn==-1)
        ...
        case ARRAY:
            ...
            else
            {
                ...
                T->num = 1;
                T->width = 1;
                struct ASTNode * bar = T;
                while(bar->kind==ARRAY)
                {
                    T->width*=bar->ptr[1]->type_int;
                    bar = bar->ptr[0];
                }
                T->width*=4;//无脑乘以4
  ...
  if (T)
	...
	case ASSIGNOP:
                ...
                if (T->ptr[0]->kind!=ID&&T->ptr[0]->kind!=ARRAY){
                    ...
                    opn1.kind=T->ptr[1]->kind==ARRAY?ARRAY:ID;   strcpy(opn1.id,symbolTable.symbols[T->ptr[1]->place].alias);//右值一定是个变量或临时变量
                    opn1.offset=symbolTable.symbols[T->ptr[1]->place].offset;
                    //printf("!!!!!!%d\n",opn1.offset);
                    if(T->ptr[1]->kind==ARRAY)//此时需要再次填入opn2
                    {
                        int foo = symbolTable.symbols[T->ptr[1]->place].info->target;
                        opn2.kind = ID;strcpy(opn2.id,symbolTable.symbols[foo].alias);
                        opn2.offset = symbolTable.symbols[foo].offset;
                    }
                    if(T->ptr[0]->kind==ID)
                    {    
    ...
    case FUNC_CALL: //根据T->type_id查出函数的定义，如果语言中增加了实验教材的read，write需要单独处理一下
                ...
                T->place=fill_Temp(newTemp(),LEV,T->type,'T',T->offset+T->width);//去掉-width
                ...
                while (T0) {  //处理所以DEC_LIST结点
                    ...
                    if (T0->ptr[0]->kind==ID){
                        ...
                        if(p)
                        {
                            ...
                            T0->ptr[0]->width = 1;
                            struct ASTNode * bar = T0->ptr[0];
                            while(bar->kind==ARRAY)
                            {
                                T0->ptr[0]->width*=bar->ptr[1]->type_int;
                                bar = bar->ptr[0];
                            }
                            T0->ptr[0]->width*=4;//无脑乘以4
